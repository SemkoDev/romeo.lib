...
class BasePage extends Base {
  ...
  getJobs () {
    return Object.values(this.opts.queue.jobs)
      .filter(j => j.opts && j.opts.page === this.opts.index)
  }
      ...
      const addressPromise = new Promise((resolve, reject) => {
        iota.api.ext.getAddresses(
          ...
          async (err, addresses) => {
            ...
            cached.length < addresses.length && this.applyAddresses(addresses);
            resolve(this);
          }
        )
      });
  ...
  sendTransfers (transfers, inputs, message, messageFail, priority) {
    ...
    const sendPromise = new Promise((resolve, reject) => {
      iota.api.sendTransfer(
        seed, IOTA_DEPTH, IOTA_MWM, transfers,
        { inputs },
        (err, result) => {
          if (err) {
            return reject(err)
  ...
  restoreAddresses (addresses, message, messageFail) {
    ...
    return Promise.all(addresses.map(async address => (
      await this.sendTransfers([ { address, value: 0 } ], null, message, messageFail)
    )))
  }
